...
class ParticleSystem(object):
   ...
   def __init__(self, position, particleType, parallax=1):
      self._particles = []
   ...
   def update(self, seconds):
      ...
      if len(self._particles) < self._maxParticles and self._timer < 0:
         self._timer = (random.random() / 2 + 0.5) * self._frequency
         self._particles.append(self._particleType(self.getPosition(), self._initialVelocity, parallax=self._parallax))
...
class RangedParticleSystem(ParticleSystem):
   def __init__(self, position, particleType, parallax=1):
      super().__init__(position, particleType, parallax=parallax)
...
class ShowerSystem(ParticleSystem):
   def __init__(self, position, parallax=1):
      super().__init__(position, Shower, parallax=parallax)
   ...
   def _yMove(self, particle):
      particle.velocity.y += 0.1 * self._initialVelocity.y
...
class RainSystem(RangedParticleSystem):
   def __init__(self, position, parallax=1):
      super().__init__(position, Rain, parallax=parallax)
...
class ThinRainSystem(RangedParticleSystem):
   def __init__(self, position, parallax=1):
      super().__init__(position, ThinRain, parallax=parallax)
...
class SnowSystem(RangedParticleSystem):
   def __init__(self, position, parallax=1):
      super().__init__(position, Snow, parallax=parallax)
...
class SmokeSystem(ParticleSystem):
   def __init__(self, position, parallax=1):
      super().__init__(position, Smoke, parallax=parallax)
