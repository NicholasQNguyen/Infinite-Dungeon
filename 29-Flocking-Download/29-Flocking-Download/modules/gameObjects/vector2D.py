...
class Vector2(object):
   ...
   def clip(self, length):
      """If the current magnitude of self is longer than length it scales,
         otherwise does nothing."""
      if self.magnitude() > length:
         self.scale(length)
   ...
   def __add__(self, other):
      ...
      if type(other) in [type(self), list, tuple]:
         clone = Vector2(*self)
         clone[0] += other[0]
         clone[1] += other[1]
   ...
   def __neg__(self):
      """Returns the negation of self, -x"""
   ...
   def getAngle(self):
      ...
      if self.x == 0:
         ...
         addition += math.atan(self.y / self.x)
