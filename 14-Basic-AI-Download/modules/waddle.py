...
class WaddleDee(Mobile):
   ...
   def think(self, kirby):
      distance = kirby.getPosition() - self.getPosition()
      if distance.magnitude() < self._sight:
         if kirby.getPosition().x < self.getPosition().x:
            self._state.manageState("right", self)
         else:
            self._state.manageState("left", self)
         if kirby.getPosition().y < self.getPosition().y:
