...
class RepeatingSprite(Drawable):
   ...
   def __init__(self, dimensions, imageName, position=Vector2(0,0), offset=None, parallax=1):
      ...
      for x in range(0, self._dimensions[0], self._tileSize[0]):
         for y in range(0, self._dimensions[1], self._tileSize[1]):
            ...
            if self._showBox:
               pygame.draw.rect(self._image, (0,255,0), pygame.Rect((x, y), self._tileSize), 1)
...
class EfficientBackground(RepeatingSprite):
   ...
   def update(self):
      if self._parallax != 0:
         topLeft = Drawable.WINDOW_OFFSET * self._parallax
...
class MovingBackground(EfficientBackground):
   ...
   def update(self, ticks):
      self._position = self._position + self._velocity * ticks
...
class Floor(Drawable):
   def __init__(self, width):
      super().__init__("",Vector2(0,168))
      self._image = pygame.Surface((width,16))
   
      self._image.fill((255,255,255))
      
      for x in range(0,width,32):
         pygame.draw.rect(self._image,(190,190,190),pygame.Rect(x,0, 16,16))
